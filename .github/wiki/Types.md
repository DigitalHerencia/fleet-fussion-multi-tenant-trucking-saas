...
This document provides a comprehensive reference for all TypeScript types, interfaces, and validation schemas used throughout the FleetFusion application.
...
export function isUserRole(value: unknown): value is UserRole {
  return isString(value) && ['admin', 'manager', 'dispatcher', 'driver', 'compliance', 'accountant', 'viewer'].includes(value)
}
...
export function isLoadStatus(value: unknown): value is LoadStatus {
  return isString(value) && ['pending', 'assigned', 'in_transit', 'delivered', 'cancelled'].includes(value)
}
```
...
*This type reference is automatically updated as the codebase evolves. For specific implementation details, consult the source files in the `types/` directory.*
