...
export default function SignInPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.status === "complete") {
        ...
        if (returnBackUrl) {
          ...
          // Default redirect to /org-selection
          logger.info("SignIn: redirecting to default", { default: "/org-selection" });
    ...
    } catch (error) { // Changed 'error' to 'err' to match usage below
      logger.error("SignIn: error", error);
      // Clerk error handling
      // Check if error is a Clerk error object
      const clerkError = Array.isArray(error) && error.length > 0 ? error[0] : null;

      if (error === 401 && error) { // Example, adjust based on actual Clerk error structure for session_exists
        router.replace("/dashboard");
        return;
      }
      ...
      } else if (error) { // Fallback for other types of errors
