...
export async function FinancialMetricsServer({
  ...
  const revenueTimelineData = await getDailyRevenueTimeline(
    companyId,
    startDate,
    endDate,
  );
  ...
  const financialSummary = {
    ...
    expenses: {
      ...
      previous: Math.round(
        Number(previousMetricsData?.totalRevenue || 0) * 0.74,
      ),
      change: previousMetricsData?.totalRevenue
        ? (
    ...
    profit: {
      ...
      previous: Math.round(
        Number(previousMetricsData?.totalRevenue || 0) * 0.26,
      ),
      change: previousMetricsData?.totalRevenue
        ? (
            ((totalRevenue -
    ...
    margin: {
      current:
        totalRevenue > 0
          ...
          : "0.0",
      previous:
        previousMetricsData?.totalRevenue &&
