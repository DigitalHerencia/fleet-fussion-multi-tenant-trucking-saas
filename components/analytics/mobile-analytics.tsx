...
export function MobileAnalytics({ data, orgId, timeRange }: MobileAnalyticsProps) {
  ...
  const chartData = useMemo(() => {
    const timeSeriesData = data?.timeSeriesData || [];    const labels = timeSeriesData.map((item: any) => 
  ...
  // Quick action cards
  const quickActions = [
    { icon: <Filter className="h-5 w-5" />, label: 'Filters', action: () => {} },
    { icon: <Share className="h-5 w-5" />, label: 'Share', action: () => {} },
    { icon: <Download className="h-5 w-5" />, label: 'Export', action: () => {} },
    { icon: <BookmarkPlus className="h-5 w-5" />, label: 'Save', action: () => {} }
  ];
  ...
  const MetricCard = ({ title, value, change, icon, index }: any) => (
    ...
    >
      <Card className={`${selectedCard === index ? 'ring-2 ring-blue-500' : ''} transition-all duration-200`}>
