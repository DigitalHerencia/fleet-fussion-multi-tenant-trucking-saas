...
export async function PerformanceMetricsServer({
  ...
  const timelineData = await getDailyRevenueTimeline(
    companyId,
    startDate,
    endDate,
  );
    ...
    return {
      ...
      loads: Math.round(
        Number(item.revenue || 0) /
          (Number(metricsData?.avgRatePerMile || 1000) * 0.8),
      ),
      // Calculate approximate miles based on revenue and average rate per mile
      miles: Math.round(
        Number(item.revenue || 0) / Number(metricsData?.avgRatePerMile || 1),
      ),
  ...
  const comparisonData = {
    ...
    miles: {
      current: Math.round(
        Number(metricsData?.totalRevenue || 0) /
          Number(metricsData?.avgRatePerMile || 3),
      ),
      previous: Math.round(
