...
export async function PerformanceMetricsServer({ companyId, timeRange }: PerformanceMetricsServerProps) {
  ...
  const formattedTimelineData = timelineData.map(item => {
    ...
    return {
      ...
      loads: Math.round(Number(item.revenue || 0) / (Number(metricsData?.avgRatePerMile || 1000) * 0.8)),
  ...
  const loadCountChange = previousMetricsData?.loadCount && metricsData?.loadCount
    ? ((Number(metricsData.loadCount) - Number(previousMetricsData.loadCount)) / Number(previousMetricsData.loadCount)) * 100
    : 0
  ...
  const comparisonData = {
    ...
    miles: {
      current: Math.round(Number(metricsData?.totalRevenue || 0) / Number(metricsData?.avgRatePerMile || 3)),
      previous: Math.round(Number(previousMetricsData?.totalRevenue || 0) / Number(previousMetricsData?.avgRatePerMile || 3)),
      change: "6.7"
    },
    onTimeDelivery: {
      current: 94.2,
