...
export async function VehicleUtilizationServer({
  ...
  const vehicleData = await getVehicleUtilization(
    companyId,
    startDate,
    endDate,
  );
  ...
  const formattedVehicleData = vehicleData.map((vehicle) => {
    ...
    return {
      number:
        vehicle.unitNumber || `V-${Math.floor(Math.random() * 900) + 100}`, // Use unit number or generate random
  ...
  return (
    <VehicleUtilization timeRange={timeRange} data={formattedVehicleData} />
  );
}
