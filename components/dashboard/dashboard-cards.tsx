...
import { Truck, Users, Package, Calendar, AlertTriangle, DollarSign, TrendingUp, Clock } from "lucide-react"
    ...
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mt-6">
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">{kpis.upcomingMaintenance} scheduled maintenance</p>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">{kpis.utilizationRate} utilization rate</p>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">${kpis.revenuePerMile}/mile average</p>
        </CardContent>
      </Card>

      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Total Miles (30d)</CardTitle>
          <TrendingUp className="h-4 w-4 text-muted-foreground" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{kpis.totalMiles.toLocaleString()}</div>
          <p className="text-xs text-muted-foreground">
            {Math.round(kpis.totalMiles / (kpis.activeVehicles || 1))} miles per vehicle avg
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Inspections (30d)</CardTitle>
          <AlertTriangle className="h-4 w-4 text-muted-foreground" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{kpis.recentInspections}</div>
          <p className="text-xs text-muted-foreground">{kpis.failedInspections} failed inspections</p>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">Awaiting assignment or pickup</p>
        </CardContent>
      </Card>
    </div>
  )
