...
export function LoadDetailsDialog({
    ...
    const [selectedDriverId, setSelectedDriverId] = useState(load.driver?.id || "not_assigned")
    const [selectedVehicleId, setSelectedVehicleId] = useState(load.vehicle?.id || "not_assigned")
    const [selectedTrailerId, setSelectedTrailerId] = useState(load.trailer?.id || "not_assigned")
    ...
    const handleAssign = async () => {
        ...
        try {
            // Convert "not_assigned" values to null
            const driverId = selectedDriverId === "not_assigned" ? null : selectedDriverId
            ...
            if (result.success) {
                toast({
                    title: "Success",
                    description: "Load assignment updated successfully"
                })
                onClose()
            } else {
                toast({
                    title: "Error",
                    description: result.error || "Failed to update load assignment",
                    variant: "destructive"
                })
            }
        } catch (error) {
            ...
            toast({
                title: "Error",
                description: "An unexpected error occurred while assigning the load",
                variant: "destructive"
            })
        } finally {
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            <div>
                                                ...
                                                <div className="font-medium">
                                                    {load.rate ? formatCurrency(load.rate) : "N/A"}
                                                </div>
                                            </div>
                                            <div>
                                                <Label className="text-xs flex items-center gap-1">
                            ...
                            <CardContent className="space-y-4">
                                <div className="flex justify-between items-center">
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-sm text-muted-foreground">
                                            Add BOL, POD, or other documents
                                        </p>
                                    </div>
                                ...
                                <div className="border rounded-md p-4">
                                    <div className="text-center text-muted-foreground">
                                        ...
                                        <p className="text-sm">
                                            Upload documents to keep track of important paperwork
                                        </p>
                                    </div>
                                </div>
                            </CardContent>
                ...
                <DialogFooter className="flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                        {load.status === "pending" && (
                            ...
                            >
                                {isUpdatingStatus ? "Updating..." : "Mark Assigned"}
                            </Button>
                        )}
