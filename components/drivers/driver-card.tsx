...
export function DriverCard({ driver, onClick }: DriverCardProps) {
  ...
  return (
    <Card
      className="cursor-pointer bg-black hover:shadow-md transition-shadow"
      {...(onClick ? { onClick } : {})}
    >
        ...
        <div className="w-full space-y-1">
          ...
          {driver.licenseExpiration && (
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs text-muted-foreground">License Exp:</span>
              </div>
              <span className="text-xs font-medium">{formatDate(driver.licenseExpiration)}</span>
            </div>
          )}
          {driver.medicalCardExpiration && (
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1">
                <Calendar className="h-3.5 w-3.5 text-muted-foreground" />
                <span className="text-xs text-muted-foreground">Medical Exp:</span>
              </div>
              <span className="text-xs font-medium">{formatDate(driver.medicalCardExpiration)}</span>
            </div>
          )}
        </div>
      </CardFooter>
