...
export function DriverAssignmentDialog({ driverId, open, onClose, currentAssignment, onAssigned }: DriverAssignmentDialogProps) {
  ...
  async function handleUnassign() {
    ...
    try {
      ...
      if (result.success) {
        toast({ title: "Driver unassigned", description: "Assignment removed." });
        onAssigned?.();
        onClose();
      } else {
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent>
        ...
        <DialogFooter className="flex gap-2">
          <Button onClick={handleAssign} disabled={form.submitting} type="button">Assign</Button>
          <Button onClick={handleUnassign} disabled={form.submitting} type="button" variant="outline">Unassign</Button>
