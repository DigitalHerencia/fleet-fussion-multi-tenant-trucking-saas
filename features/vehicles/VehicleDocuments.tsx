...
export default function VehicleDocuments({ vehicleId, documents: initialDocuments }: VehicleDocumentsProps) {
      ...
      {showUpload && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <form className="bg-white dark:bg-zinc-900 p-6 rounded-lg shadow-lg space-y-4 w-full max-w-md" onSubmit={handleUpload}>
            <h3 className="text-lg font-semibold">Upload Vehicle Document</h3>
            <input name="name" placeholder="Document Name" className="input w-full" required />
            <input name="type" placeholder="Type (e.g. Registration)" className="input w-full" required />
