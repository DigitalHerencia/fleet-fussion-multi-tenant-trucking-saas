...
export async function getIftaReports(params: {
  year: number;
  quarter: number;
  limit?: number;
}) {
  ...
  if (!parsed.success) {
    console.error(
      "[IFTA-Actions] getIftaReports validation error:",
      parsed.error.flatten(),
    );
    throw new Error("Invalid parameters for getIftaReports");
  }
  try {
    const reports = await fetchIftaReports({
      year: params.year,
...
export async function updateIftaTrip(id: string, formData: FormData) {
  try {
    ...
    await db
      ...
      .set({
        ...
        fuelPurchases: data.fuelPurchases
          ? JSON.stringify(data.fuelPurchases)
          : null,
      })
    ...
    return {
      success: false,
      error:
        error instanceof Error ? error.message : "Failed to update IFTA trip",
...
export async function deleteIftaTrip(id: string) {
  try {
    ...
    return {
      success: false,
      error:
        error instanceof Error ? error.message : "Failed to delete IFTA trip",
...
export async function createIFTAAction(formData: FormData) {
  try {
    ...
    return {
      success: false,
      error:
        err instanceof Error ? err.message : "Failed to create IFTA record",
      errors: { form: ["Failed to create IFTA record"] },
    };
  }
}

export async function createFuelPurchaseAction(undefined: undefined, formData: FormData) {
  try {
    ...
    return {
      success: false,
      error:
        err instanceof Error ? err.message : "Failed to create fuel purchase",
