...
export async function createInsurancePolicy(formData: FormData) {
  ...
  const data = {
    ...
    premium: parsed.data.premium !== undefined && parsed.data.premium !== null
...
export async function updateInsurancePolicy(id: string, formData: FormData) {
  ...
  const insuranceToUpdate = {
    ...
    startDate: parsed.data.startDate ? new Date(parsed.data.startDate).toISOString() : undefined,
    endDate: parsed.data.endDate ? new Date(parsed.data.endDate).toISOString() : undefined,
  };
  const [policy] = await db.update(insurancePolicies).set(insuranceToUpdate).where(eq(insurancePolicies.id, id)).returning()
  return { success: true, policy }
}
