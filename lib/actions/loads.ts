...
export async function createLoad(formData: FormData) {
  ...
  try {
    ...
    const pickupDate = validatedFields.pickupDate ? new Date(validatedFields.pickupDate) : undefined
    const deliveryDate = validatedFields.deliveryDate ? new Date(validatedFields.deliveryDate) : undefined

    // Convert numeric fields to strings for DB insertion if they exist
    const weightString = validatedFields.weight !== undefined ? String(validatedFields.weight) : undefined;
    const rateString = validatedFields.rate !== undefined ? String(validatedFields.rate) : undefined;
    const milesString = validatedFields.miles !== undefined ? String(validatedFields.miles) : undefined;

    await db.insert(loads).values({
      ...validatedFields,
      weight: weightString,
      rate: rateString,
      miles: validatedFields.miles !== undefined ? validatedFields.miles : undefined,
...
export async function updateLoad(id: string, formData: FormData) {
  ...
  try {
    ...
    // Convert numeric fields to strings for DB update if they exist
    const weightString = validatedFields.weight !== undefined ? String(validatedFields.weight) : undefined;
    const rateString = validatedFields.rate !== undefined ? String(validatedFields.rate) : undefined;
    const milesString = validatedFields.miles !== undefined ? String(validatedFields.miles) : undefined;
    ...
    const existingLoad = await db.query.loads.findFirst({
      where: (loads, { eq, and }) => and(eq(loads.id, id), eq(loads.companyId, company.id)),
    })
