...
export async function createSettlement(formData: FormData): Promise<ApiResult<any>> {
    try {
        ...
        const [settlement] = await db
            .insert(settlements)
            .values(settlementToInsert)
            .returning()
        return { success: true, data: settlement }
    } catch (error) {
        console.error("[SettlementActions] createSettlement error:", error)
        return {
            success: false,
...
export async function updateSettlement(id: string, formData: FormData): Promise<ApiResult<any>> {
    try {
        const parsed = settlementSchema.safeParse(Object.fromEntries(formData))
        if (!parsed.success) {
            return {
        ...
        const [settlement] = await db
            .update(settlements)
            .set(settlementToUpdate)
            .where(eq(settlements.id, id))
            .returning()
        return { success: true, data: settlement }
    } catch (error) {
        console.error("[SettlementActions] updateSettlement error:", error)
        return {
            success: false,
