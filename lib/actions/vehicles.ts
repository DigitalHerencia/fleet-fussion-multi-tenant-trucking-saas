...
export async function updateVehicle(id: string, formData: FormData) {
  ...
  try {
    ...
    const existingVehicle = await db.query.vehicles.findFirst({
      where: (vehicles, { eq, and }) => and(eq(vehicles.id, id), eq(vehicles.organizationId, user.organizationId)),
    })
...
export async function getVehicles() {
  ...
  try {
    const allVehicles = await db.query.vehicles.findMany({
      where: (vehicles, { eq }) => eq(vehicles.organizationId, user.organizationId),
      with: {
        loads: {
          limit: 5,
...
export async function getAvailableTractors() {
  ...
  try {
    const availableTractors = await db.query.vehicles.findMany({
      where: (vehicles, { eq, and }) => 
        and(
          eq(vehicles.organizationId, user.organizationId), 
          eq(vehicles.status, 'active'),
          eq(vehicles.type, 'tractor')
        ),
      orderBy: (vehicles, { asc }) => [asc(vehicles.unitNumber)],
    })
...
export async function getAvailableTrailers() {
  ...
  try {
    const availableTrailers = await db.query.vehicles.findMany({
      where: (vehicles, { eq, and }) => 
        and(
          eq(vehicles.organizationId, user.organizationId), 
          eq(vehicles.status, 'active'),
          eq(vehicles.type, 'trailer')
        ),
      orderBy: (vehicles, { asc }) => [asc(vehicles.unitNumber)],
    })
