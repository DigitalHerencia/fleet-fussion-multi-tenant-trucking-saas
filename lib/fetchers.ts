...
export async function getKPIs(companyId: string): Promise<KPI> {
  logger.debug("getKPIs called", { companyId });
  try {
    ...
    const activeVehicles = await db.query.vehicles.findMany({
      where: (vehicles, { eq, and }) =>
        and(eq(vehicles.companyId, companyId), eq(vehicles.status, "active")),
      columns: { id: true },
    });
    ...
    const activeDrivers = await db.query.drivers.findMany({
      where: (drivers, { eq, and }) =>
        and(eq(drivers.companyId, companyId), eq(drivers.status, "active")),
      columns: { id: true },
    });

    // Loads in last 30 days
    const recentLoads = await db.query.loads.findMany({
      where: (loads, { eq, and, gte }) =>
        and(eq(loads.companyId, companyId), gte(loads.createdAt, thirtyDaysAgo)),
      columns: { id: true, status: true, miles: true, rate: true },
    });

    // Calculate load statistics
    const totalLoads = recentLoads.length;
    const completedLoads = recentLoads.filter(
      (load) => load.status === "completed",
    ).length;
    const pendingLoads = recentLoads.filter(
      (load) => load.status === "pending",
    ).length;
    const inTransitLoads = recentLoads.filter(
      (load) => load.status === "in_transit",
    ).length;

    // Calculate revenue and miles
    const totalMiles = recentLoads.reduce(
      (sum, load) => sum + (load.miles || 0),
      0,
    );
    const totalRevenue = recentLoads.reduce(
    ...
    const recentInspections = await db.query.inspections.findMany({
      where: (inspections, { eq, and, gte }) =>
        and(
          ...
          gte(inspections.date, thirtyDaysAgo),
        ),
    ...
    logger.info("getKPIs success", { companyId });
    return {
      ...
      utilizationRate:
        activeVehicles.length > 0
          ? (completedLoads / activeVehicles.length).toFixed(2)
          : 0,
...
export async function getLoads(
  ...
  logger.debug("getLoads called", { companyId, status, driverId });
  try {
    ...
    logger.info("getLoads success", { companyId, status, driverId });
    return query;
  } catch (error) {
    logger.error("getLoads error", error);
    throw error;
  }
}
...
export async function getVehicles(companyId: string, status?: string) {
  logger.debug("getVehicles called", { companyId, status });
  try {
    ...
    logger.info("getVehicles success", { companyId, status });
    return query;
  } catch (error) {
    logger.error("getVehicles error", error);
    throw error;
  }
}
...
export async function getDrivers(companyId: string, status?: string) {
  logger.debug("getDrivers called", { companyId, status });
  try {
    const query = db.query.drivers.findMany({
      ...
      orderBy: (drivers, { asc }) => [
        ...
        asc(drivers.firstName),
      ],
    });

    logger.info("getDrivers success", { companyId, status });
    return query;
  } catch (error) {
