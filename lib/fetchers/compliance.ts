...
export async function getDriverComplianceData(companyId: string) {
  try {
    ...
    const violationsQuery = await db
      .select({
    ...
    return driversData.map((d) => {
      ...
      if (licenseExpiry && isBefore(new Date(licenseExpiry), new Date())) {
        ...
        medicalExpiry &&
        isBefore(new Date(medicalExpiry), new Date())
      ) {
        status = "Non-Compliant";
      } else if (
        licenseExpiry &&
        isBefore(new Date(licenseExpiry), warningThreshold)
      ) {
        status = "Warning";
      } else if (
        medicalExpiry &&
        isBefore(new Date(medicalExpiry), warningThreshold)
      ) {
        status = "Warning";
      }
      ...
      return {
        ...
        medicalExpiry: medicalExpiry
          ? new Date(medicalExpiry).toISOString().split("T")[0]
          : "N/A",
        lastHosViolation,
...
export async function getVehicleComplianceData(companyId: string) {
  try {
    ...
    return vehiclesData.map((v) => {
      ...
      const lastInspectionDate = inspection
      ...
      const today = new Date();
      const registrationExpiry = addDays(
        today,
...
export async function getComplianceDocumentData(companyId: string) {
  try {
    ...
    return documents.map((doc) => {
      ...
      if (
        assignedTo === "Driver" &&
        documents.filter((d) => d.driverId).length > 1
      ) {
        assignedTo = "All Drivers";
      }
      if (
        assignedTo === "Vehicle" &&
        documents.filter((d) => d.vehicleId).length > 1
      ) {
...
export async function getComplianceSummaryMetrics(companyId: string) {
  try {
    ...
    const compliantDrivers = driversData.filter(
    ...
    const compliantVehicles = vehiclesData.filter(
    ...
    const compliantDocuments = documentsData.filter(
      (d) => d.status === "Complete",
    ).length;
 ...
 */
export async function getUpcomingComplianceDeadlines(
  companyId: string,
  ...
  try {
    ...
    const driverDeadlines = await db
      .select({
        id: drivers.id,
        firstName: drivers.firstName,
        lastName: drivers.lastName,
    ...
    driverDeadlines.forEach((driver) => {
      if (
        ...
        const daysUntil = Math.ceil(
          (new Date(driver.licenseExpiration).getTime() - today.getTime()) /
            (1000 * 60 * 60 * 24),
        );
        deadlines.push({
          type: "Driver CDL",
      ...
      if (
        ...
        const daysUntil = Math.ceil(
          (new Date(driver.medicalCardExpiration).getTime() - today.getTime()) /
            (1000 * 60 * 60 * 24),
        );
        deadlines.push({
    ...
    const documentDeadlines = await db
      .select({
        id: complianceDocuments.id,
        name: complianceDocuments.name,
        expirationDate: complianceDocuments.expirationDate,
    ...
    iftaDueDates.forEach((iftaDate) => {
      ...
      const iftaYear =
        iftaDate.quarter === "Q4" ? currentYear - 1 : currentYear;
      const iftaDueDate = new Date(iftaYear, iftaDate.month - 1, iftaDate.day);

      // Check if the due date is within the next 30 days
      if (
        isAfter(iftaDueDate, today) &&
        isBefore(iftaDueDate, thirtyDaysFromNow)
      ) {
        const daysUntil = Math.ceil(
          (iftaDueDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24),
        );
        deadlines.push({
          type: "IFTA Filing",
