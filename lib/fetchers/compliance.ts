...
export async function getVehicleComplianceData(companyId: string) {
    try {
        ...
        return vehiclesData.map(v => {
            ...
            const today = new Date()
            const registrationExpiry = addDays(today, Math.floor(Math.random() * 365) + 30)
                .toISOString()
                .split("T")[0]

            // Determine if there are defects (simplified)
            const defects =
                inspection && inspection.status === "failed" ? "Major - Requires Attention" : "None"
...
export async function getComplianceSummaryMetrics(companyId: string) {
    try {
        ...
        const vehicleComplianceRate =
            totalVehicles > 0 ? Math.round((compliantVehicles / totalVehicles) * 100) : 0
        ...
        const documentComplianceRate =
            totalDocuments > 0 ? Math.round((compliantDocuments / totalDocuments) * 100) : 0
