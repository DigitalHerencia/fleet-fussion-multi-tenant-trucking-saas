...
export const getIftaTrips = cache(async function getIftaTrips({
    ...
    try {
        ...
        if (startDate && endDate) {
            conditions.push(gte(iftaTrips.startDate, startDate), lte(iftaTrips.startDate, endDate))
        }
...
export const getIftaReports = cache(async function getIftaReports({
    ...
    try {
        ...
        return reports.map(report => ({
            ...
            avgMpg:
                report.totalMiles && report.totalGallons && Number(report.totalGallons) > 0
 ...
 */
export async function getIftaSummaryMetrics(companyId?: string, year?: number, quarter?: number) {
    try {
        let cId = companyId
        if (!cId) {
            cId = await getCompanyId()
        ...
        const currentTotals = await db
            ...
            .from(iftaTrips)
            .where(
                and(eq(iftaTrips.companyId, cId), between(iftaTrips.startDate, startDate, endDate))
            )
        ...
        const gallonsChange =
            previous.totalGallons > 0
                ? (
                      ((current.totalGallons - previous.totalGallons) / previous.totalGallons) *
                      100
                  ).toFixed(1)
                : "0.0"
