...
import { iftaTrips, iftaReports, vehicles, drivers, loads } from "@/db/schema"
import { eq, and, between, desc, sql, gte, lte } from "drizzle-orm"
import { isBefore, endOfQuarter, startOfQuarter, format } from "date-fns"
  ...
  try {
    ...
    return trips.map(trip => ({
      ...
      driver: trip.driverFirstName && trip.driverLastName ? 
        `${trip.driverFirstName} ${trip.driverLastName}` : 'Unassigned',
      vehicle: trip.vehicleUnitNumber || 'Unknown',
      jurisdictionData: trip.jurisdictionData ? JSON.parse(trip.jurisdictionData as string) : null,
      fuelPurchases: trip.fuelPurchases ? JSON.parse(trip.fuelPurchases as string) : null,
    }))
  } catch (error) {
...
export const getIftaReports = cache(async function getIftaReports({
  ...
  try {
    ...
    return reports.map(report => ({
      ...
      avgMpg: report.totalMiles && report.totalGallons && Number(report.totalGallons) > 0 
...
export async function getIftaSummaryMetrics(
  ...
  try {
    ...
    const currentQuarter = quarter || Math.floor((currentDate.getMonth() / 3) + 1) 
    
    // Calculate start and end dates for the current quarter
    const startDate = startOfQuarter(new Date(currentYear, (currentQuarter - 1) * 3, 1))
    ...
    const currentTotals = await db
      .select({
        totalMiles: sql<string>`COALESCE(SUM(${iftaTrips.totalMiles}), 0)`.as("totalMiles"),
        totalGallons: sql<string>`COALESCE(SUM(${iftaTrips.gallons}), 0)`.as("totalGallons"),
        uniqueJurisdictions: sql<number>`COUNT(DISTINCT ${iftaTrips.state})`.as("uniqueJurisdictions")
      })
      .from(iftaTrips)
      .where(and(
    ...
    const milesChange = previous.totalMiles > 0 
      ? ((current.totalMiles - previous.totalMiles) / previous.totalMiles * 100).toFixed(1)
      : '0.0'
      
    const gallonsChange = previous.totalGallons > 0 
      ? ((current.totalGallons - previous.totalGallons) / previous.totalGallons * 100).toFixed(1)
      : '0.0'
    
    // Calculate average MPG
    const currentMpg = current.totalGallons > 0 ? (current.totalMiles / current.totalGallons).toFixed(1) : '0.0'
    const previousMpg = previous.totalGallons > 0 ? (previous.totalMiles / previous.totalGallons).toFixed(1) : '0.0'
    const mpgChange = parseFloat(previousMpg) > 0
