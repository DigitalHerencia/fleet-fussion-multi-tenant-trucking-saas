...
setInterval(() => {
  ...
  for (const [key, { timestamp }] of ipRequestStore.entries()) {
    if (now - timestamp > 60 * 1000) { // Remove entries older than 1 minute
...
export function rateLimit(req: NextRequest) {
  ...
  headers.set('X-RateLimit-Remaining', Math.max(0, RATE_LIMIT_REQUESTS - currentRequests.count).toString());
  headers.set('X-RateLimit-Reset', (Math.ceil((currentRequests.timestamp + RATE_LIMIT_WINDOW_MS) / 1000)).toString());
