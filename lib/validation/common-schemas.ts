...
export const phoneRegex = new RegExp(
  /^\+?[1-9]\d{1,14}$/
); // Basic E.164-like regex, ensure to escape backslashes in strings for RegExp constructor if used
...
export const isoDateString = z.string().datetime({ message: "Invalid ISO 8601 date format, e.g., YYYY-MM-DDTHH:mm:ssZ or YYYY-MM-DDTHH:mm:ss.sssZ" });

// For optional dates that might come as empty strings from forms, or be null/undefined
export const optionalIsoDateString = z.union([
...
export const optionalFileSchema = z.custom<File>((val) => val instanceof File, "Invalid file provided.").optional().nullable();
