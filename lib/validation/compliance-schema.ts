...
export const HosStatusEnum = z.enum(["ON_DUTY", "OFF_DUTY", "SLEEPER_BERTH", "DRIVING"])
...
export const hosLogSchema = z.object({
    ...
    vehicleId: z.string().uuid({ message: "Invalid vehicle ID." }).optional().nullable(),
    startTime: z.string().datetime({ message: "Invalid start time." }),
    endTime: z.string().datetime({ message: "Invalid end time." }).optional().nullable(),
...
export const defectSchema = z.object({
    description: z.string().min(3, { message: "Defect description is too short." }).max(500),
...
export const complianceDocumentSchema = z.object({
    ...
    driverId: z.string().uuid({ message: "Invalid driver ID." }).optional().nullable(),
    vehicleId: z.string().uuid({ message: "Invalid vehicle ID." }).optional().nullable(),
    type: ComplianceDocumentTypeEnum,
    name: z.string().min(3, { message: "Document name is too short." }).max(255),
    description: z.string().max(1000).optional().nullable(),
    issueDate: z.string().datetime({ message: "Invalid issue date." }).optional().nullable(),
    expiryDate: z.string().datetime({ message: "Invalid expiry date." }).optional().nullable(),
