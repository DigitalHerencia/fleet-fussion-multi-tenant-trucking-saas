/**
 * RBAC-only types for FleetFusion
 *
 * Removes all ABAC types, permissions, and claims. Only includes role and minimal user/org metadata.
 */

// Canonical system role definition
export type SystemRole = 'admin' | 'manager' | 'driver' | 'viewer';

// Use system roles as the canonical role definition
...
// Extended user context with RBAC data
export interface UserContext {
  name: string | undefined;
  userId: string;
  organizationId: string;
  role: UserRole;
  permissions: never[]; // No permissions in RBAC-only model
  email: string;
  firstName?: string;
  lastName?: string;
  profileImage?: string;
