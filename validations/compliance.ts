...
export const createMaintenanceSchema = z.object({
  ...
  category: z.enum(["engine", "transmission", "brakes", "tires", "electrical", "hvac", "body", "other"]),
  description: z.string().min(1, "Description is required"),
...
export const maintenanceFilterSchema = z.object({
  vehicleId: z.string().optional(),
  type: z.array(z.enum(["preventive", "corrective", "inspection", "recall", "warranty"])).optional(),
  status: z.array(z.enum(["scheduled", "overdue", "in_progress", "completed", "cancelled"])).optional(),
  ...
  sortBy: z.enum(["scheduledDate", "dueDate", "completedDate", "priority", "cost", "vehicle"]).optional(),
  sortOrder: z.enum(["asc", "desc"]).optional(),
  page: z.number().int().min(1).optional(),
  limit: z.number().int().min(1).max(100).optional(),
})
...
export const safetyEventFilterSchema = z.object({
  type: z.array(z.enum(["accident", "violation", "inspection", "complaint", "incident", "citation"])).optional(),
