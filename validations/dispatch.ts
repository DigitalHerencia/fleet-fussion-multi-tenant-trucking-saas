...
// Rate validation schema (for detailed breakdown, not for DB 'rate' field)
...
// Helper for rate field: must be number or numeric string, and >= 0
const rateField = z.union([z.number(), z.string()]).refine(
  (val) => {
    if (typeof val === "number") return val >= 0;
    if (typeof val === "string") return /^\d+(\.\d+)?$/.test(val) && parseFloat(val) >= 0;
    return false;
  },
  { message: "Rate must be a positive number or numeric string" }
);
...
export const createLoadSchema = z.object({
  ...
  rate: rateField,
  rateDetails: rateDetailsSchema.optional(), // For UI/logic only, not stored in DB
...
export const loadStatusUpdateSchema = z.object({
  ...
  source: z.enum(["system", "driver", "dispatcher", "customer", "eld"]).default("dispatcher"),
})
...
export const loadFilterSchema = z.object({
  ...
  equipmentType: z.array(z.enum(["dry_van", "reefer", "flatbed", "step_deck", "lowboy", "tanker", "container", "other"])).optional(),
  ...
  sortBy: z.enum(["pickupDate", "deliveryDate", "rate", "miles", "priority", "status", "customer", "createdAt"]).optional(),
...
export const bulkLoadOperationSchema = z.object({
  operation: z.enum(["delete", "update_status", "assign_driver", "assign_vehicle", "add_tags", "remove_tags"]),
...
export const loadDocumentSchema = z.object({
  ...
  type: z.enum(["bol", "pod", "invoice", "receipt", "permit", "contract", "rate_confirmation", "other"]),
  category: z.enum(["pickup", "delivery", "administrative", "billing", "compliance"]),
